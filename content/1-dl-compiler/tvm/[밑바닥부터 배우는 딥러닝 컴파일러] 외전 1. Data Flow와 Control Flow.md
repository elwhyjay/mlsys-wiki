...
![이미지](images/zerodl_extra1_dataflow_controlflow/img_01.png)위 TensorFlow 프로그램에 대응하는 dataflow 그래프
...
![이미지](images/zerodl_extra1_dataflow_controlflow/img_02.png)Control Flow의 한 예
...
![이미지](images/zerodl_extra1_dataflow_controlflow/img_03.png)여기서 A의 값을 가져오는 것은 단지 그것을 다시 피드백하기 위한 것이다

그리고 이러한 Python 레벨의 Control Flow는 computation graph에 표현되지 않는다. 즉,

![이미지](images/zerodl_extra1_dataflow_controlflow/img_04.png)노란색 부분은 computation graph에서 사실상 삭제된 것이다. 초기의 TensorFlow는 이런 제어 로직을 표현할 수 없었기 때문이다
...
![이미지](images/zerodl_extra1_dataflow_controlflow/img_05.png)요약하면 PyTorch에서 Python 레벨의 control flow를 포함한 모델을 ONNX로 export하면 control flow가 사라진다. 이를 보존하려면 TorchScript 모델로 export하거나 script 모델 기반의 export 방식을 사용하는 것을 권장한다
...
![이미지](images/zerodl_extra1_dataflow_controlflow/img_06.png)TensorFlow는 네이티브 control flow를 위한 몇 가지 operator를 제공한다
...
![이미지](images/zerodl_extra1_dataflow_controlflow/img_07.png)제어 의존성으로 인한 경쟁 상태
...
![이미지](images/zerodl_extra1_dataflow_controlflow/img_08.png)인위적으로 bot과 top의 의존 관계를 추가하여, 지정한 operator가 먼저 완료되도록 한다
...
![이미지](images/zerodl_extra1_dataflow_controlflow/img_09.png)의존 관계를 추가한 후의 computation graph는 이런 모습이다

## 조건 분기

다음으로 조건 분기를 살펴보자. 이 절 첫머리에서 제시한 예시를 TensorFlow는 어떻게 처리할까?

![이미지](images/zerodl_extra1_dataflow_controlflow/img_10.png)TensorFlow는 두 가지 조건 제어 op, 즉 tf.cond와 tf.case를 제공한다

다음 코드는 tf.cond를 활용해 조건 분기를 구현한 것으로, a < b가 참일 때 out을 평가하면 tf.add(3, 3)이 실행되고, 그렇지 않으면 tf.square(3)이 실행된다.

![이미지](images/zerodl_extra1_dataflow_controlflow/img_11.png)tf.cond로 조건 분기 구현

위 코드는 다음과 동일하다. `tf.cond(a < b, lambda: tf.add(3, 3), lambda: tf.sqaure(3))`

생성된 computation graph는 다음과 같다.

![이미지](images/zerodl_extra1_dataflow_controlflow/img_12.png)조건 control flow가 포함된 computation graph

병렬 분기가 많을 때는 tf.case를 사용해 처리할 수 있다. 예를 들어,

![이미지](images/zerodl_extra1_dataflow_controlflow/img_13.png)병렬 조건 분기가 2개를 초과할 때는 tf.case로 제어한다

## 반복

TensorFlow는 `tf.while_loop`를 제공하여 반복 블록을 구성한다. RNN과 같은 구조에서 이런 요구가 있을 것 같다. 예를 들어,

![이미지](images/zerodl_extra1_dataflow_controlflow/img_14.png)tf.while_loop는 반복 control flow를 구현하여 RNN과 같은 computation graph 구조의 제어 로직을 해결할 수 있다

다음 코드는 기본적인 반복 예시로, 100회 반복한다.

![이미지](images/zerodl_extra1_dataflow_controlflow/img_15.png)tf.while_loop를 사용해 static graph에서 반복 control flow를 구현한다
...
![이미지](images/zerodl_extra1_dataflow_controlflow/img_16.png)Module을 직접 export하면 Python 레벨의 제어 로직이 사라진다(즉, for 루프가 완전히 펼쳐진다). 이는 PyTorch가 ONNX를 export할 때 기본적으로 tracing 메커니즘을 사용하기 때문이다
...
![이미지](images/zerodl_extra1_dataflow_controlflow/img_17.png)PyTorch 모델에서 Python 레벨로 정의된 Control Flow가 보존되었다
